...
class BRepNet(LightningModule):
    ...
    def __init__(self, opts):
        ...
        self.opts = opts
        kernel = file_utils.load_json(opts.kernel)
        input_feature_metadata = file_utils.load_json(opts.input_features)
        ...
        if segment_names_file is not None:
            self.segment_names = file_utils.load_json(segment_names_file)
    ...
    def forward(self, Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf):
        ...
        """
        face_embeddings = self.create_face_embeddings(Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf) # type: ignore
    ...
    def test_epoch_end(self):
        # Получаем outputs из атрибута класса
        outputs = getattr(self, "_test_outputs", [])
        output = self.collate_epoch_outputs(outputs)
        num_faces = output["total_num_faces"]
