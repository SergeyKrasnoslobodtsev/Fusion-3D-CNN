...
def concat_face_features(data):
    """ Объединяет признаки граней и координаты точек на гранях в один массив признаков граней."""
    face_features = np.asarray(data["face_features"])
    face_point_grids = np.asarray(data["face_point_grids"])
    if face_point_grids.ndim == 3:
        face_point_grids = face_point_grids.reshape(face_point_grids.shape[0], -1)
    return np.concatenate([face_features, face_point_grids], axis=1)
...
class CustomDataset(Dataset):
            ...
            self.data.append(
                {
                    ...
                    "face_features": brepnet_feats["face_features"], # признаки граней
                    "face_point_grids": brepnet_feats["face_point_grids"], # координаты точек на гранях
                    "edge_features": brepnet_feats["edge_features"], # признаки рёбер
                    # "edge_point_grids": brepnet_feats["edge_point_grids"], # координаты точек на рёбрах
                    "coedge_features": brepnet_feats["coedge_features"], # признаки ко-рёбер
                    "coedge_point_grids": brepnet_feats["coedge_point_grids"], # координаты точек на ко-рёбрах
                    "coedge_lcs": brepnet_feats["coedge_lcs"], # локальные системы координат ко-рёбер
                    "coedge_scale_factors": brepnet_feats["coedge_scale_factors"], # масштабные коэффициенты ко-рёбер
                    "coedge_reverse_flags": brepnet_feats["coedge_reverse_flags"], # флаги реверса ко-рёбер
                    "coedge_to_next": brepnet_feats["coedge_to_next"],  # индексы следующих ко-рёбер
                    "coedge_to_mate": brepnet_feats["coedge_to_mate"],  # индексы сопряжённых ко-рёбер
                    "coedge_to_face": brepnet_feats["coedge_to_face"],  # индексы граней для ко-рёбер
                    "coedge_to_edge": brepnet_feats["coedge_to_edge"]   # индексы рёбер для ко-рёбер
                }
            )
            if compute_stats:
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "coedge_scale_factors": data["coedge_scale_factors"], # масштабные коэффициенты ко-рёбер
            "coedge_reverse_flags": data["coedge_reverse_flags"], # флаги реверса ко-рёбер
            "coedge_to_next": data["coedge_to_next"],  # индексы следующих ко-рёбер
            "coedge_to_mate": data["coedge_to_mate"],  # индексы сопряжённых ко-рёбер
            "coedge_to_face": data["coedge_to_face"],  # индексы граней для ко-рёбер
            "coedge_to_edge": data["coedge_to_edge"]   # индексы рёбер для ко-рёбер
        }